...
if [ -n "$BITS" ]; then
    JAVA_MEM_OPTS=" -server -Xmx400m -Xms400m -Xmn400m -Xss256k -XX:+DisableExplicitGC -XX:+UseConcMarkSweepGC -XX:+CMSParallelRemarkEnabled -XX:+UseCMSCompactAtFullCollection -XX:LargePageSizeInBytes=128m -XX:+UseFastAccessorMethods -XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 "
else
    JAVA_MEM_OPTS=" -server -Xms400m -Xmx400m -XX:PermSize=256m -XX:MaxPermSize=512M -XX:SurvivorRatio=2 -XX:+UseParallelGC "
fi
